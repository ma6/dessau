 ...
 *
 * Contents: site header disclosure · breadcrumb collapsed middle · table of
 *   contents · content navigation · banner dismiss
 *
 * The menu component needs no JavaScript at all: it uses the `popover`
 * attribute with `popovertarget`, which the platform handles. The breadcrumb's
 * collapsed middle reuses that same markup once it decides a level needs to
 * be hidden — only the decision itself is script.
 */
(function (global) {
  'use strict';
  ...
  /* =========================================================================
     ...
     Every level is authored — the collapsing is entirely the script's doing,
     the same division of labour as `.dds-toc`'s highlight: the consumer
     writes a plain, complete list and the enhancement decides what the row
     has room for. There is no query that can ask "did my siblings' text make
     this row wrap" — only actual layout can, so the decision has to be made
     here rather than in CSS.

     Only levels that do not fit are hidden, and the row is measured before
     every decision rather than patched incrementally: growing back out
     restores every level first, THEN re-measures, so a trail that fits again
     is never left short because of a narrower measurement that no longer
     holds.

     Levels closest to the current page are kept longest. A shortened trail is
     read for "where under X am I", which "… › Documents › Drainage survey"
     answers and "Projects › … › Drainage survey" — the immediate parent
     hidden, the root kept — does not. First and last are never collapsed:
     first is the root of the hierarchy, last is where the user is, and a
     breadcrumb with neither showing would keep its two least useful ends.

     A hidden level is not removed, only marked `data-dds-breadcrumb-hidden` —
     which drops it from layout AND from the accessibility tree, same as the
     platform's own `hidden` would, but under a name specific to this
     component. The reference's own "Show markup" panel strips it back off a
     captured sample for exactly that reason (`reference-tools.js`,
     `RUNTIME_ATTRIBUTES`): plain `hidden` already means something an author
     writes on purpose elsewhere on this page (`.dds-primary-nav`'s own
     no-script fallback), so reusing it here would make the sample lie about
     which trail was authored versus which one the script decided to show.

     The popover holds a CLONE of each hidden level's link, not the original —
     duplicated rather than moved, which sounds like the wrong call until the
     accessibility tree is checked rather than assumed: the original stays
     `data-dds-breadcrumb-hidden` (out of the tree) and the clone lives inside
     a closed popover (also out of the tree, same as any `.dds-menu` before
     it opens), so exactly one copy is ever exposed, never both. Wired exactly
     like the menu's own trigger — `popovertarget`, and the popover as its
     very next sibling so it becomes the implicit anchor — but left-aligned
     with `.dds-menu-align-start`, because this trigger sits at the start of a
     row, not in the corner `.dds-menu`'s default alignment is tuned for. The
     whole trigger-and-popover `<li>` carries `data-dds-generated`
     (agent/conventions.md): it encloses no authored content of its own, only
     the button and the clones, so removing it for the code sample takes
     nothing real with it.

     Without JavaScript: every level is a working link and the list wraps,
     exactly as `.dds-breadcrumb` already behaves on its own. */

  var BREADCRUMB_WORDING = {
    en: { showPath: 'Show full path' },
    de: { showPath: 'Vollständigen Pfad anzeigen' },
  };

  DDS.register('breadcrumb', '.dds-breadcrumb', function (nav) {
    var ol = nav.querySelector(':scope > ol');
    if (!ol) return;

    // The full, original trail — first and last are never hidden or cloned;
    // only `middle` ever is.
    var items = Array.prototype.slice.call(ol.children);
    if (items.length <= 2) return; // nothing between first and last to hide

    var middle = items.slice(1, -1);

    var trigger = document.createElement('button');
    trigger.type = 'button';
    trigger.className = 'dds-breadcrumb-ellipsis';
    trigger.textContent = '…';
    trigger.setAttribute('aria-label', DDS.utils.wording(nav, BREADCRUMB_WORDING).showPath);

    var popoverId = DDS.utils.uniqueId('dds-breadcrumb-path');
    trigger.setAttribute('popovertarget', popoverId);

    var popover = document.createElement('div');
    popover.className = 'dds-menu dds-menu-align-start';
    popover.id = popoverId;
    popover.setAttribute('popover', '');

    var popoverList = document.createElement('ul');
    // Safari drops list semantics from a styled list outside a landmark
    // (agent/accessibility.md) — the same reason `.dds-menu`'s own examples
    // carry it.
    popoverList.setAttribute('role', 'list');
    popover.appendChild(popoverList);

    var ellipsisItem = document.createElement('li');
    ellipsisItem.setAttribute('data-dds-generated', '');
    ellipsisItem.appendChild(trigger);
    ellipsisItem.appendChild(popover);

    /**
     * Does every currently-shown level sit on the same line?
     *
     * Compared against the FIRST one rather than checked pairwise, so a break
     * anywhere in the row is caught, not only one between two particular
     * items. A hidden `<li>` is `display: none` (see the CSS) and would
     * measure as a zero rect at the origin, so it is filtered out first
     * rather than compared against.
     */
    function fitsOneLine() {
      var visible = Array.prototype.slice.call(ol.children).filter(function (li) {
        return !li.hasAttribute('data-dds-breadcrumb-hidden');
      });
      if (visible.length < 2) return true;
      var top = visible[0].getBoundingClientRect().top;
      return visible.every(function (li) {
        return Math.abs(li.getBoundingClientRect().top - top) < 1;
      });
    }

    function hide(li) {
      li.setAttribute('data-dds-breadcrumb-hidden', '');

      var link = li.querySelector(':scope > a');
      if (!link) return;
      var clone = document.createElement('li');
      var a = link.cloneNode(true);
      a.classList.add('dds-menu-item');
      clone.appendChild(a);
      popoverList.appendChild(clone);
    }

    /** Every level shown again, ellipsis gone, popover emptied. */
    function restore() {
      if (ellipsisItem.parentNode) ellipsisItem.remove();
      popoverList.replaceChildren();
      middle.forEach(function (li) {
        li.removeAttribute('data-dds-breadcrumb-hidden');
      });
    }
    ...
    function collapse() {
      ...
      // `middle[0]` never moves — only ever hidden in place — so it is
      // always a valid reference node here regardless of what the previous
      // measurement collapsed.
      ol.insertBefore(ellipsisItem, middle[0]);

      // Hidden from the root down: the level immediately before the current
      // page is the one most worth keeping, because a shortened trail is
      // read for "where under the first entry am I".
      for (var i = 0; i < middle.length && !fitsOneLine(); i += 1) {
        hide(middle[i]);
      }
    }
    ...
    /**
     * Re-measure when the row's available width changes — a sidebar closing,
     * a container query elsewhere on the page resolving differently, the
     * reference's own width switcher, none of which fire a `resize` event.
     *
     * Guarded to WIDTH specifically, not "any size change": hiding or
     * restoring levels changes `ol`'s HEIGHT (one line versus two), and an
     * unguarded observer watching its own element would see that as a fresh
     * resize, call `collapse()` again, and loop against itself indefinitely.
     * The row's available width never changes as a result of which levels are
     * shown — only external layout changes it — so comparing width alone
     * breaks that loop at the source instead of debouncing around it.
     */
    if (typeof ResizeObserver !== 'undefined') {
      var lastWidth = null;
